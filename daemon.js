...
export async function main(ns) {
    ...
    async function startup(ns) {
        ...
        if (competingDaemons.length > 0) { // We expect only 1, due to this logic, but just in case, generalize the code below to support multiple.
            ...
            await ns.await ns.sleep(loopInterval); // The game can be slow to kill scripts, give it an extra bit of time.
        }
        ...
        try {
            if (4 in dictSourceFiles && options['initial-study-time'] > 0) {
                ...
                try {
                    ...
                    if (!university)
                        ...
                        if (startedStudying)
                            await ns.await ns.sleep(studyTime * 1000); // Wait for studies to affect Hack XP. This will often greatly reduce time-to-hack/grow/weaken, and avoid a slow first cycle
                        else
            ...
            if (!xpOnly) {
                ...
                while (maxXpCycles-- > 0 && Date.now() - start < maxXpTime * 1000) {
                    ...
                    if (cycleTime)
                        await ns.await ns.sleep(cycleTime);
                    else
    ...
    async function runStartupScripts(ns) {
        ...
        for (const script of asynchronousHelpers.filter(s => !s.isLaunched)) {
            ...
            if (++launched > 1) await ns.await ns.sleep(1); // If we successfully launch more than 1 script at a time, yeild execution a moment to give them a chance to complete, so many aren't all fighting for temp RAM at the same time.
    ...
    async function doTargetingLoop(ns) {
        ...
        do {
            loops++;
            if (loops > 0) await ns.await ns.sleep(loopInterval);
            try {
                let start = Date.now();
    ...
    async function arbitraryExecution(ns, tool, threads, args, preferredServerName = null, useSmallestServerPossible = false, allowThreadSplitting = null) {
        ...
        // IDEA: "home" is more effective at await ns.grow() and await ns.weaken() than other nodes (has multiple cores) (TODO: By how much?)
        ...
        for (let i = 0; i < rootedServersByFreeRam.length && remainingThreads > 0; i++) {
            ...
            if (targetServer.name != daemonHost && !(await tool.existsOnHost(targetServer))) {
                ...
                //await ns.await ns.sleep(5); // Workaround for Bitburner bug https://github.com/danielyxie/bitburner/issues/1714 - newly created/copied files sometimes need a bit more time, even if awaited
            }
    ...
    /** @param {NS} ns
     * Grind hack XP by filling a bunch of RAM with await ns.hack() / await ns.grow() / await ns.weaken() against a relatively easy target */
    ...
    async function waitForCycleEnd(ns, server, maxWaitTime = 200, waitInterval = 5) {
        ...
        while (stillBusy = server.isXpFarming(false) && maxWaitTime > 0) {
            await ns.await ns.sleep(waitInterval); // Sleep a very short while, then get a fresh process list to check again whether the process is done
    ...
    async function scheduleHackExpCycle(ns, server, percentOfFreeRamToConsume, verbose, advancedMode, allocatedServer = null, singleServer = false) {
        ...
        try {
            ...
            if (advancedMode) { // We get the most XP by using max possible hack threads while keeping money just above 0 (so that we get full await ns.hack() exp)
            ...
            } else { // If the server is at min-security, we should farm await ns.grow(), since it takes less time (80%) than await ns.weaken(). Once at max-money, grow will no longer reduce security.
            ...
            if (advancedMode) { // Need to keep server money above zero, and security at minimum to farm xp from await ns.hack();
                ...
                const securityHardeningToCombat = Math.max(effectiveHackThreads * hackThreadHardening + growThreadsNeeded * growthThreadHardening, // Security that will be incurred await ns.hack() + await ns.grow() threads
