...
export async function main(ns) {
    ...
    if (10 in dictSourceFiles) {
        ...
        await ns.await ns.sleep(500); // Give it time to make its initial purchases. Note that we do not block on the process shutting down - it will keep running.
    }
    ...
    pid = await runCommand(ns, `while(ns.singularity.upgradeHomeCores()); { await ns.await ns.sleep(10); }`, '/Temp/upgrade-home-ram.js');
    await waitForProcessToComplete(ns, pid, true); // Wait for the script to shut down, indicating it has bought all it can.
    ...
    while (ticksWithoutPurchases < options['ticks-to-wait-for-additional-purchases'] && (Date.now() < maxWait)) {
        ...
        while ((Date.now() - start <= 200) && lastMoney == await refreshMoney())
            await ns.await ns.sleep(10); // Wait for game to tick (money to change) - might happen sooner than 200ms
    ...
    */
    ...
    if (options.reset || options['install-augmentations']) {
        ...
        await ns.await ns.sleep(1000); // Pause for effect?
