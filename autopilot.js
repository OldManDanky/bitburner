...
export async function main(ns) {
    ...
    async function main_start(ns) {
        ...
        while (keepRunning) {
            ...
            await ns.await ns.sleep(options['interval']);
        }
    }
    ...
    async function maybeDoCasino(ns, player) {
        ...
        if (pid) {
            await waitForProcessToComplete(ns, pid);
            await ns.await ns.sleep(10000); // Give time for this script to be killed if the game is being restarted by casino.js
    ...
    async function maybeInstallAugmentations(ns, player) {
        ...
        if (pid) {
            ...
            await ns.await ns.sleep(1000); // If we've been scheduled to be killed, awaiting an NS function should trigger it?
