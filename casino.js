...
export async function main(ns) {
    ...
    async function saveGame() {
        if (saveSleepTime) await ns.await ns.sleep(saveSleepTime);
        ...
        while (true) {
            ...
            while (winLoseTie == null) {
                ...
                try {
                    ...
                    await ns.await ns.sleep(1); // Yield for an instant so the game can update and process events (e.g. deal the next card)
...
async function reload(ns) {
    ...
    while (attempts++ <= 5) {
        ...
        await ns.await ns.sleep(options['save-sleep-time']); // Yield execution for an instant incase the game needs to finish a save or something
        location.reload(); // Force refresh the page without saving
        await ns.await ns.sleep(10000); // Keep the script alive to be safe. Presumably the page reloads before this completes.
...
async function click(ns, button) {
    ...
    if (sleepDelay > 0) await ns.await ns.sleep(sleepDelay);
...
async function internalfindWithRetry(ns, xpath, expectFailure, maxRetries, customErrorMessage = null) {
    try {
        ...
        if (options['find-sleep-time'])
            await ns.await ns.sleep(options['find-sleep-time']);
        ...
        while (attempts++ <= maxRetries) {
            ...
            if (attempts > 1) {
                ...
                await ns.await ns.sleep(retryDelayMs);
