...
export async function main(ns) {
    ...
    if (startupScript) {
        ...
        if (ns.run(startupScript, 1, ...startupArgs)) {
            ...
            await ns.await ns.sleep(1000); // Give time for the accompanying script to start up and consume its required RAM footprint.
        } else
    ...
    while (true) {
        await ns.await ns.sleep(lastLoopSuccessful ? 10 : 1000); // Only sleep a short while between charges if things are going well
